...
of the reproducibility of neural networks training (Deep Learning content). This notebook has been run on an Ubuntu tower thi a NVIDIA 
...
In the `Reproductibility-MNIST-CNN-GPU` directory we show the same study with a Convolutional Neural Network (__CNN__).
## How to use
...
-> this will download and install all the required Python module in the sub-directory `.venv`
